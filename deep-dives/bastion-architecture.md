...
For remote/external access, a separate **Access Manager (WAM)** component is placed in front of one or more Bastions as an **HTML5 reverse proxy** — a single HTTPS entry point, no browser plug-in. WAM is required for **Session Invite** and cross-Bastion audit. See the [portfolio's WAM section](../docs/00-overview/product-portfolio.md#access-manager-web-portal--single-point-of-access--gateway).
...
- Cross-reference: [../docs/00-overview/product-portfolio.md](../docs/00-overview/product-portfolio.md) (architecture/deployment/HA subsection, with its own sources).
