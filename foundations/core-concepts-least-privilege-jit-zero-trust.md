...
flowchart LR
    ...
    R --> A --> G --> E
    Notes["If DENIED at step 2<br/>→ no access, request<br/>logged.<br/>If approval TIMES OUT<br/>→ request expires,<br/>nothing granted.<br/>Net effect: ZERO standing privilege<br/>between tasks (→ ZSP)."]
    R -.-> Notes
    A -.-> Notes
    G -.-> Notes
